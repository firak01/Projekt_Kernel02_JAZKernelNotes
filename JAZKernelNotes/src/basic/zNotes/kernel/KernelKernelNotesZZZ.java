...
public class KernelKernelNotesZZZ extends KernelUseObjectZZZ implements IKernelNotesConstZZZ{ //ObjectZZZ{
	...
	//Session & Context	
	private Session objSession = null;            //Session, die durch AgentBase.getSession() gewonnen wird. Enth�lt Context Informationen
	//20060921 nun soll der NotesContextProvider alles liefern, darum keine spezielle DebugSession mehr 
	//private Session objSessionDebug = null;  //Session, die durch NotesFactory.createSession(Username, Password) generiert wird. Enth�lt KEINE Context Informationen.
																		 //Erm�glicht aber im Debug Modus eines externen Debuggers den Zugriff auf andere Datenbanken.
	...
	private String sAgentNameFaked=null; //Falls ein Servlet verwendet wird, ist dieser Name zu �bergeben.
	...
	private Document docStoreApplication = null; // Das Storedokument der Applikation, um sich �ndernde Werte festzuhalten. Z.B. fortlaufende Nummern.
	...
	public NotesContextProviderZZZ getNotesContextProvider() throws ExceptionZZZ{
		//Merke: Im Fall, das es ein echter notesagent ist, wird der Contextprovider entweder sofort im Konstruktor �bergeben
		//            oder der Context ist nun aus der Session heraus zu ermitteln.
		//Merke2: Bei Servlets, etc. ohne echten Context, muss der Agentenname (gefaked) �bergeben werden.
	...
	public Session getSession() throws ExceptionZZZ{
		...
		if(this.objSession==null){
			System.out.println(ReflectCodeZZZ.getMethodCurrentName() + ", objSession==null");
			 ...
			 //Dies ist dann die L�sung f�r servlets und alles weitere, das kein echter Agent ist.
...
public String getApplicationKeyDefault() {
	//Merke: Diese Function muss immer vom customizing Teil �berschreiben werden.
  ...
  public String ProfileKernelMaskCompute(){
		...
		main:{
			...
			try{
				if(this.docProfileApplication==null){
					...
					System.out.println("KernelKernelNotesZZZ." + ReflectCodeZZZ.getMethodCurrentName() + ", Hole Profildokument mit Schl�ssel: Mask='" + sMask + "' und Key='" + sKey + "' aus Datenbank " + dbCur.getServer() + "!!" + dbCur.getFilePath());
				...
				//Auch wenn da schon ein Dokument ist, pr�fen, ob es ein Konfigurationsdokument ist
  		...
  		try{
			if(this.dbKernelCalling == null){
				if(getFlag("UseContextByKernel")==true){
					String sLog = ReflectCodeZZZ.getMethodCurrentName() + ", dbKernelCalling==null, UseContextByKernel-Flag==true; Hole die Datenbank �ber den NotesContextProviderZZZ";
					System.out.println(sLog);
					dbFunction = this.getNotesContextProvider().getDb();				
				}else{
					String sLog = ReflectCodeZZZ.getMethodCurrentName() + ", dbKernelCalling==null, UseContextByKernel-Flag==false; Hole die Datenbank �ber den NotesContextProviderZZZ";
					System.out.println(sLog);
	...
	/** Holt f�r den Key aus der Applikation ein Notesdokument, in dem applikationsspezifische Informationen gespeichert werden.
	...
	* lindhaueradmin; 12.12.2006 08:22:07
	...
	/**Analog zum LotusScript-Kernel wird nun eine Methode angeboten, mit der ein Dokument default Reader/Author - Eintr�ge bekommen kann.
	 *  Es werden die Felder AuthorZZZ/ReaderZZZ, Author + ApplicationKey, bzw. Reader + ApplicationKey angelegt und mit den Standardrollen gef�llt.
	 *  
	 *  Standardrollen:
	 *  [ZZZAuthor]
	 *  [ZZZReader]
	 ...
	 *  [ZZZServer]
	 *  
	 *  entsprechend f�r die Application z.B. beim ApplicationKey VIA
	 *  [VIAAuthor]
	 *  
	 *  
	 *  
...
public KernelNotesLogZZZ getKernelNotesLogObject() throws ExceptionZZZ{
	  if(this.objKernelNotesLog==null){
		  //+++ Das Kernel-Log
		this.objKernelNotesLog = new KernelNotesLogZZZ(this, "");  //Merke: in diesem Spezialkonstruktor wird dann ein Typecast auf KernelNotesZZZ durchgef�hrt
	  }
...
public Database getDBByAlias(String sAlias, String sFlagcontrol) throws ExceptionZZZ{
	...
	main:{
		...
		System.out.println(ReflectCodeZZZ.getMethodCurrentName() + ", Hole Datenbankhandle f�r " + sServer + "!!" + sDBPath);
	...
	/** Also: Den Modulalias �bergeben und anhand des aktuellen (ggf. "faked") Agentennamens den Namen der Section erhalten
...
TargetPath=c:\tempfgl\SI\EDM_Interface\file00ZZZ
	...
	public String getAgentParameterCurrentZ(String sZAliasIn, String sParamIn)throws ExceptionZZZ{
		//TODO gleich methode, bei der das Object FileIniZZZ schon �bergeben wird, soll bei wiederholten Aufrufen die Performance steigern.
		String sReturn = null;
		String sMethod; IKernelZZZ objKernel=null;
		main:{
			try{
			String sZAlias;
			String sEnvironmentVariable; //wird hier nicht als parameter �bergeben sondern muss aus dem AgentenContext ermittelt werden.
			String sParam;    
			check:{
				objKernel = this.getKernelObject();
				if(objKernel == null){
	...
	* lindhaueradmin; 29.09.2006 08:35:34
	...
	public String getEnvironmentZ(String sZAliasIn, String sEnvironmentVariableIn) throws ExceptionZZZ{
		//TODO eine methode, bei der das FileIniZZZ-Object schon �bergeben wird, Performance.
		String sReturn = null;
		String sMethod; IKernelZZZ objKernel=null;
		main:{
	...
	private String DirectoryCompute_(String sBaseDirectory, String sDirectory, String sFlagControl) throws ExceptionZZZ{
			...
			main:{
				...
				if (dir.exists() == false){
					...
					//Vorhandenen Namen pr�fen, ob er auch tats�chlich ein Verzeichnis ist.						
			...
			//Das Verzeichnis (Falls nicht anders in dem KernelZZZ, �ber die ini-Datei, definiert)
...
public String getSystemNumber() throws ExceptionZZZ{
	try{
		if(StringZZZ.isEmpty(this.sSystemNumber)){
			//Falls es sich z.B. um ein Servlet handelt, wird der Context gefaked, ansonsten soll man die SystemNumber aus dem Applikation-Profildokument auslesen k�nnen.
