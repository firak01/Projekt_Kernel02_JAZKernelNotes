...
startpoint:
    ...
    String sFuncCur = "NotesMain";  //Merke: Todo: Eine Klasse f�r das Handeln der Codeposition schreiben;
    PrintWriter out = getAgentOutput(); // For Output  //Merke: Todo - Eine Klasse f�r das Handeln des Otuputs schreiben.
                                                            //Diese soll sowohl Exceptions verarbeiten k�nnen
...
main:
			try{                                            //Merke: Eine Klasse schreiben, die sich von Exception ableitet
                                                      ...
                                                      //Merke2: Alle Z-Objekte m�ssen von einer Klasse ZZZObject erben.
                                                      //        Alle Funktionen m�ssen die ZZZException auswerfen.
				// get the session
         ...
         // Merke: wird mal wichtig, wenn es darum geht Profildokumente zu holen !!!
			...
			KernelNotesZZZ objKernelNotes = new KernelNotesZZZ(session, objKernel,"","", (String[])null);
             ...
             out.println("Agent l�uft in der Datenbank: '" + sTitle + "'");
             out.flush(); //Sicherstellen, dass der Outputcache leer ist.

			 String sUsername = session.getUserName();
			 out.println("Session f�r : '" + sUsername + "'");
             out.flush(); //Sicherstellen, dass der Outputcache leer ist.
...
*/
		...
		//f�r den Test auf fgl01: 		String sDirectoryTarget = new String("e:\\temp\\albis\\");
        ...
        //String sFileSource = doc.getItemValueString("FileNameZZZ");
         //o.k. wenn der Server als Dienst unter dem Administratorennamen l�uft: String sFileSource = "\\\\Atilla\\temp\\test.txt";
     ...
     //Mappen der Dateinamen und kopieren               
       ...
       if(saFileSource[i] != null){ 	
        ...
        // ToDo: Hier nach dem Targetnamen fragen und ggf. eine Zahl von 001-999 anh�ngen.
	...
	public String NameFileTargetCompute(String sPathTarget, String sFileTarget, int iExpandLengthIn) throws ExceptionZZZ {
		/*
		 * Anh�ngen einer 3-stelligen Ziffer (von 001 - 999) an die Datei,
		 * falls der Dateiname schon vorhanden ist.
		 * Falls diese schon vorhanden ist, wird zur n�chsten Ziffer gegangen		 * 
		 */
		 String sFunction = "";
		 String sEnding = new String("");   //die Endung z.B. txt
		 String sFileOnly = new String(""); //nur der Dateinamsbestandteil (also ohne Suffix)
		 int iFileOnlyLength = 0; // L�nge des Dateinamens ohne das Suffix z.B. .txt
		 int iFileTargetLength = 0; //L�nge des Dateinamens inklusive Suffix
		 int iExpandLength = 0;   //L�nge der angestrebten Dateinamensendung
