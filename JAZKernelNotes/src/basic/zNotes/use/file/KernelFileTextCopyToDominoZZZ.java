...
public class KernelFileTextCopyToDominoZZZ extends KernelNotesUseObjectZZZ {//ObjectZZZ implements AssetKernelNotesZZZ{
	...
	public boolean startit() throws ExceptionZZZ{
		...
		//	try{
				 ...
				 */																
				//TODO Ein Konstruktor bekommt die entsprechenden Dateinamen in einem Array �bergeben. Hier werden dann diese Dateien Kopiert und verschoben.
	...
	//Use only one Log-File for all relevant entries
		IKernelZZZ objK = objKernelNotes.getKernelObject();
	 ...
	 if(this.saFileSource==null){
	 	//Fall: +++ Dateinamen werden in einer speziellen Funktion initalisiert oder es werden alle Dateien eines Verzeichnisses gew�hlt
		...
		//+++ Fall: Die Dateinamen wurden mit dem gesamten Pfad �bergeben
		if(this.saFileSource.length<=0){
		   objKernelNotesLog.writeLog("No file found to process",this,"",1);
		   bFunction = false;
		   break main;
